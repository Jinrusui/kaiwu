...
def run_episodes(envs, agents, logger, monitor):
    ...
    model_id = ['31694','41847','55774','60633']
    ...
    while True:
        ...
        #if k<=0.7:
        opponent_agent = model_id[-1]
        
        
        #     opponent_agent = model_id[1]
        # else:opponent_agent = model_id[2]
        ...
        for i in range(agent_num):
            ...
            for key, value in reward.items():
                ...
                if key in weighted_total_reward_dicts[i]:
                    weighted_total_reward_dicts[i][key] += value * GameConfig.REWARD_WEIGHT_DICT[key]
                else:
                    if key != 'reward_sum':
                        weighted_total_reward_dicts[i][key] = value * GameConfig.REWARD_WEIGHT_DICT[key]
        ...
        while True:
                ...
                for key, value in reward.items():
                    ...
                    if key in weighted_total_reward_dicts[i]:
                        if key != 'reward_sum':
                            ...
                            weighted_total_reward_dicts[i][key] += value
                    else:
                        if key != 'reward_sum':
                            weighted_total_reward_dicts[i][key] = value * GameConfig.REWARD_WEIGHT_DICT[key]
                        else:
            ...
            if terminated or truncated:
                ...
                logger.info(
                    f"episode_{episode_cnt} weighted_total_reward_dicts:{weighted_total_reward_dicts}"
                )
                ...
                monitor_data = {
                    ...
                    "diy_1": round(weighted_total_reward_dicts[train_agent_id]['strengthen'], 2),
                    "diy_2":round(weighted_total_reward_dicts[train_agent_id]["defend_tower"],2),
                    "diy_3":round(weighted_total_reward_dicts[train_agent_id]['skill_hit_count'],2),
                    "diy_4":round(weighted_total_reward_dicts[train_agent_id]["game_status"],2),
                    "diy_5":round(weighted_total_reward_dicts[train_agent_id]["approach_tower"],2),
                    "diy_6":round(weighted_total_reward_dicts[train_agent_id]["flash"],2),
                }
                # monitor_data = {
                #     "reward": round(total_reward_dicts[train_agent_id]["reward_sum"], 2),
